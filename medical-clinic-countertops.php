<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <!-- SEO Meta Tags -->
    <title>Medical Clinic Countertops FL | Griffin Quartz</title>
    <meta name="description" content="Hygienic quartz countertops for medical clinics in South Florida. Non-porous surfaces for exam rooms & dental offices. HIPAA compliant. Free estimates!">
    <meta name="keywords" content="medical clinic countertops, healthcare countertops, dental office countertops, medical office quartz, exam room countertops, lab countertops, hygienic countertops, antimicrobial countertops, medical countertops South Florida, dental countertops">
    <meta name="robots" content="index, follow">
    <link rel="canonical" href="https://griffinquartz.com/medical-clinic-countertops">

    <!-- Favicon -->
    <link rel="apple-touch-icon" sizes="180x180" href="/images/apple-touch-icon.png">
    <link rel="icon" type="image/png" sizes="32x32" href="/images/favicon-32x32.png">
    <link rel="icon" type="image/png" sizes="16x16" href="/images/favicon-16x16.png">

    <!-- Open Graph -->
    <meta property="og:title" content="Medical Clinic Quartz Countertops | Healthcare Specialists | Griffin Quartz">
    <meta property="og:description" content="Hygienic, non-porous quartz surfaces for medical and dental facilities throughout South Florida.">
    <meta property="og:url" content="https://griffinquartz.com/medical-clinic-countertops">
    <meta property="og:type" content="website">
    <meta property="og:image" content="https://griffinquartz.com/images/commercial-retail-marble-counter.webp">
    <meta property="og:image:width" content="1200">
    <meta property="og:image:height" content="630">
    <meta property="og:locale" content="en_US">
    <meta property="og:site_name" content="Griffin Quartz">

    <!-- Twitter Card -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="Medical Clinic Quartz Countertops | Healthcare Specialists | Griffin Quartz">
    <meta name="twitter:description" content="Hygienic, non-porous quartz surfaces for medical and dental facilities throughout South Florida.">
    <meta name="twitter:image" content="https://griffinquartz.com/images/commercial-retail-marble-counter.webp">

    <!-- Geo Meta Tags -->
    <meta name="geo.region" content="US-FL">
    <meta name="geo.placename" content="Boca Raton">

    <!-- Schema.org Structured Data -->
    <script type="application/ld+json">
    {
        "@context": "https://schema.org",
        "@type": "Service",
        "serviceType": "Medical Clinic Quartz Countertop Installation",
        "provider": {
            "@type": "LocalBusiness",
            "name": "Griffin Quartz",
            "telephone": "+1-720-324-1436",
            "address": {
                "@type": "PostalAddress",
                "streetAddress": "1021 S Rogers Cir #18",
                "addressLocality": "Boca Raton",
                "addressRegion": "FL",
                "postalCode": "33487"
            }
        },
        "areaServed": {"@type": "State", "name": "Florida"},
        "description": "Hygienic quartz countertop installation for medical clinics, dental offices, and healthcare facilities."
    }
    </script>

    <script type="application/ld+json">
    {
        "@context": "https://schema.org",
        "@type": "BreadcrumbList",
        "itemListElement": [
            {"@type": "ListItem", "position": 1, "name": "Home", "item": "https://griffinquartz.com"},
            {"@type": "ListItem", "position": 2, "name": "Commercial", "item": "https://griffinquartz.com/commercial"},
            {"@type": "ListItem", "position": 3, "name": "Medical Clinics", "item": "https://griffinquartz.com/medical-clinic-countertops"}
        ]
    }
    </script>

    <script type="application/ld+json">
    {
        "@context": "https://schema.org",
        "@type": "FAQPage",
        "mainEntity": [
            {
                "@type": "Question",
                "name": "Are quartz countertops sanitary enough for medical facilities?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Quartz is one of the most hygienic surface options available. Its non-porous surface prevents bacteria, mold, and mildew from penetrating, and it can be cleaned with hospital-grade disinfectants without damage. Many healthcare facilities choose quartz specifically for its sanitary properties."
                }
            },
            {
                "@type": "Question",
                "name": "Can quartz withstand medical-grade disinfectants?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Yes, quartz resists most commercial cleaning agents and hospital-grade disinfectants including diluted bleach, quaternary ammonium compounds, and hydrogen peroxide solutions. Unlike natural stone, it won't etch or degrade from chemical cleaners."
                }
            },
            {
                "@type": "Question",
                "name": "How quickly can you install countertops in an active clinic?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "We understand that medical facilities can't afford extended downtime. Most exam room or reception area installations are completed in a single day. We can work after-hours or on weekends and install in phases so you never close your entire practice."
                }
            },
            {
                "@type": "Question",
                "name": "What countertop style works best for medical offices?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Clean, light-colored quartz is popular in medical settings because it looks professional, brightens the space, and makes it easy to spot spills or contamination. Solid whites and subtle veining are the most requested for exam rooms and nurse stations."
                }
            }
        ]
    }
    </script>

    <!-- Fonts -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
    <link href="https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="styles.min.css">

    <style>
        /* Hero */
        .hero { position: relative; min-height: 600px; display: flex; align-items: center; overflow: hidden; padding: 60px 0; }
        .hero-bg { position: absolute; inset: 0; }
        .hero-bg img { width: 100%; height: 100%; object-fit: cover; }
        .hero-bg::after { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.5) 100%); }
        .hero-container { position: relative; z-index: 2; display: grid; grid-template-columns: 1fr 400px; gap: 50px; align-items: center; max-width: 1200px; margin: 0 auto; padding: 0 20px; width: 100%; }
        .hero-content { color: #fff; text-align: left; }
        .hero-badge { display: inline-block; background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); color: #fff; padding: 10px 20px; border-radius: 50px; font-weight: 600; font-size: 0.9rem; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.25); }
        .hero h1 { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 2.75rem; margin-bottom: 20px; line-height: 1.2; }
        .hero p { font-size: 1.1rem; margin-bottom: 25px; opacity: 0.9; line-height: 1.7; }
        .hero-form { background: #fff; padding: 30px; border-radius: 16px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
        .hero-form h3 { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 1.4rem; color: #000; margin-bottom: 5px; text-align: center; }
        .hero-form .form-subtitle { text-align: center; color: #666; margin-bottom: 20px; font-size: 0.95rem; }
        .hero-form .form-subtitle a { color: #FDB913; font-weight: 600; text-decoration: none; }
        .hero-form .form-group { margin-bottom: 15px; }
        .hero-form label { display: block; font-size: 0.875rem; font-weight: 500; margin-bottom: 5px; color: #333; }
        .hero-form input, .hero-form textarea, .hero-form select { width: 100%; padding: 12px; border: 1px solid #e0e0e0; border-radius: 8px; font-size: 1rem; transition: border-color 0.3s; }
        .hero-form input:focus, .hero-form textarea:focus, .hero-form select:focus { outline: none; border-color: #FDB913; }
        .hero-form .btn-full { width: 100%; padding: 14px; font-size: 1rem; }
        @media (max-width: 900px) {
            .hero-container { grid-template-columns: 1fr; }
            .hero-form { order: 1; max-width: 450px; margin: 0 auto; }
            .hero-content { text-align: center; }
        }

        /* Section Styles */
        .section-title { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 2.5rem; color: #000; margin-bottom: 1rem; }
        .section-description { color: #666; font-size: 1.1rem; margin-bottom: 2rem; }

        /* Applications Section */
        .applications-section { padding: 80px 0; background: #f8f8f8; }
        .applications-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 30px; margin-top: 40px; }
        .application-card { background: #fff; padding: 30px; border-radius: 12px; text-align: center; box-shadow: 0 4px 15px rgba(0,0,0,0.05); transition: transform 0.3s, box-shadow 0.3s; }
        .application-card:hover { transform: translateY(-5px); box-shadow: 0 8px 25px rgba(0,0,0,0.1); }
        .application-card i { font-size: 2.5rem; color: #FDB913; margin-bottom: 15px; }
        .application-card h4 { font-size: 1.1rem; color: #000; margin-bottom: 10px; }
        .application-card p { font-size: 0.9rem; color: #666; }

        /* Benefits Section */
        .benefits-section { padding: 80px 0; background: #fff; }
        .benefits-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center; margin-top: 40px; }
        .benefits-content h3 { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 1.8rem; color: #000; margin-bottom: 1rem; }
        .benefits-content p { color: #666; margin-bottom: 1.5rem; }
        .benefits-list { list-style: none; }
        .benefits-list li { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 15px; color: #333; }
        .benefits-list li i { color: #FDB913; font-size: 1.2rem; margin-top: 2px; }
        .benefits-image img { width: 100%; border-radius: 16px; box-shadow: 0 10px 40px rgba(0,0,0,0.1); }

        /* Why Choose Section */
        .why-section { padding: 80px 0; background: #000; color: #fff; }
        .why-section .section-title { color: #fff; }
        .why-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px; margin-top: 40px; }
        .why-card { background: rgba(255,255,255,0.05); padding: 30px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); }
        .why-card i { font-size: 2rem; color: #FDB913; margin-bottom: 15px; }
        .why-card h3 { font-size: 1.2rem; margin-bottom: 10px; }
        .why-card p { color: rgba(255,255,255,0.7); font-size: 0.95rem; }

        /* CTA Section */
        .cta-section { padding: 80px 0; background: linear-gradient(135deg, #FDB913 0%, #e5a811 100%); text-align: center; }
        .cta-section h2 { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 2.5rem; color: #000; margin-bottom: 15px; }
        .cta-section p { color: #000; font-size: 1.1rem; margin-bottom: 30px; max-width: 600px; margin-left: auto; margin-right: auto; }
        .cta-section .btn { background: #000; color: #fff; }
        .cta-section .btn:hover { background: #333; }

        /* Responsive */
        @media (max-width: 900px) {
            .applications-grid { grid-template-columns: repeat(2, 1fr); }
            .benefits-grid { grid-template-columns: 1fr; }
            .why-grid { grid-template-columns: 1fr; }
        }
        @media (max-width: 600px) {
            .hero h1 { font-size: 2rem; }
            .applications-grid { grid-template-columns: 1fr; }
        }
    </style>
</head>
<body>
<?php $basePath = ''; include 'includes/header.php'; ?>

    <section class="hero">
        <div class="hero-bg">
            <img src="images/commercial-retail-marble-counter.webp" alt="Medical clinic quartz countertops South Florida" width="800" height="800" loading="lazy">
        </div>
        <div class="hero-container">
            <div class="hero-content">
                <div class="hero-badge">Healthcare Specialists</div>
                <h1>Medical Clinic Quartz Countertops in South Florida</h1>
                <p>Hygienic, non-porous quartz surfaces designed for healthcare environments. From exam rooms to dental operatories, we provide countertops that meet the stringent cleanliness standards your practice demands.</p>
                <a href="tel:+17203241436" class="btn btn-primary">Call (720) 324-1436</a>
            </div>
            <div class="hero-form">
                <h3>Get Your FREE Quote</h3>
                <p class="form-subtitle">Or call <a href="tel:+17203241436">(720) 324-1436</a></p>
                <form id="heroQuoteForm">
                    <div class="form-group">
                        <label for="name">Name</label>
                        <input type="text" id="name" name="name" required>
                    </div>
                    <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" id="email" name="email" required>
                    </div>
                    <div class="form-group">
                        <label for="phone">Phone</label>
                        <input type="tel" id="phone" name="phone" required>
                    </div>
                    <div class="form-group">
                        <label for="message">Message (Optional)</label>
                        <textarea id="message" name="message" rows="3"></textarea>
                    </div>
                    <button type="submit" class="btn btn-primary btn-full">Get FREE Estimate</button>
                </form>
            </div>
        </div>
    </section>

    <section class="applications-section">
        <div class="container">
            <h2 class="section-title" style="text-align: center;">Medical & Dental Quartz Applications</h2>
            <p class="section-description" style="text-align: center; max-width: 700px; margin: 0 auto;">Purpose-built surfaces for every area of your healthcare facility.</p>

            <div class="applications-grid">
                <div class="application-card">
                    <i class="bi bi-clipboard2-pulse"></i>
                    <h4>Exam Rooms</h4>
                    <p>Easy-to-sanitize surfaces for patient examination areas and treatment rooms.</p>
                </div>
                <div class="application-card">
                    <i class="bi bi-emoji-smile"></i>
                    <h4>Dental Operatories</h4>
                    <p>Durable counters for dental chairs, instrument storage, and sterilization areas.</p>
                </div>
                <div class="application-card">
                    <i class="bi bi-eyedropper"></i>
                    <h4>Lab & Testing</h4>
                    <p>Chemical-resistant surfaces for laboratory work and specimen processing.</p>
                </div>
                <div class="application-card">
                    <i class="bi bi-door-open"></i>
                    <h4>Reception Areas</h4>
                    <p>Welcoming front desk surfaces that maintain a clean, professional appearance.</p>
                </div>
                <div class="application-card">
                    <i class="bi bi-prescription2"></i>
                    <h4>Pharmacy Counters</h4>
                    <p>Dispensing and consultation counters for in-house pharmacies.</p>
                </div>
                <div class="application-card">
                    <i class="bi bi-bandaid"></i>
                    <h4>Nurse Stations</h4>
                    <p>Functional work surfaces for charting, medication prep, and patient monitoring.</p>
                </div>
                <div class="application-card">
                    <i class="bi bi-droplet"></i>
                    <h4>Restroom Vanities</h4>
                    <p>Hygienic vanity tops for patient and staff restrooms.</p>
                </div>
                <div class="application-card">
                    <i class="bi bi-cup-hot"></i>
                    <h4>Break Rooms</h4>
                    <p>Staff lounge countertops that withstand daily use and easy cleaning.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="benefits-section">
        <div class="container">
            <div class="benefits-grid">
                <div class="benefits-content">
                    <h3>Why Quartz for Healthcare?</h3>
                    <p>Medical environments require surfaces that support infection control while maintaining a professional, calming atmosphere. Quartz countertops deliver exceptional hygiene and durability.</p>
                    <ul class="benefits-list">
                        <li><i class="bi bi-check-circle-fill"></i> <span><strong>Non-Porous Surface:</strong> Won't harbor bacteria, viruses, or pathogens that can hide in porous materials.</span></li>
                        <li><i class="bi bi-check-circle-fill"></i> <span><strong>Chemical Resistant:</strong> Withstands disinfectants, sanitizers, and cleaning chemicals without damage.</span></li>
                        <li><i class="bi bi-check-circle-fill"></i> <span><strong>Easy Sterilization:</strong> Smooth surface allows for thorough cleaning between patients.</span></li>
                        <li><i class="bi bi-check-circle-fill"></i> <span><strong>Stain Resistant:</strong> Blood, iodine, and other medical stains wipe clean without marking.</span></li>
                        <li><i class="bi bi-check-circle-fill"></i> <span><strong>Durable:</strong> Resists chips, cracks, and damage from medical equipment.</span></li>
                        <li><i class="bi bi-check-circle-fill"></i> <span><strong>Professional Aesthetics:</strong> Creates a clean, modern environment that reassures patients.</span></li>
                    </ul>
                </div>
                <div class="benefits-image">
                    <img src="images/commercial-office-lobby-modern.webp" alt="Medical office reception quartz countertops" width="800" height="800" loading="lazy">
                </div>
            </div>
        </div>
    </section>

    <section class="why-section">
        <div class="container">
            <h2 class="section-title" style="text-align: center;">Why Healthcare Facilities Choose Griffin Quartz</h2>
            <div class="why-grid">
                <div class="why-card">
                    <i class="bi bi-shield-check"></i>
                    <h3>Healthcare Experience</h3>
                    <p>Extensive experience with medical offices, dental practices, urgent care centers, and specialty clinics.</p>
                </div>
                <div class="why-card">
                    <i class="bi bi-calendar-check"></i>
                    <h3>Flexible Scheduling</h3>
                    <p>After-hours and weekend installation to avoid disrupting patient care and appointments.</p>
                </div>
                <div class="why-card">
                    <i class="bi bi-clock-history"></i>
                    <h3>Fast Turnaround</h3>
                    <p>Minimize practice downtime with efficient templating, fabrication, and installation.</p>
                </div>
                <div class="why-card">
                    <i class="bi bi-file-earmark-check"></i>
                    <h3>Compliance Aware</h3>
                    <p>Understanding of healthcare facility requirements and building codes.</p>
                </div>
                <div class="why-card">
                    <i class="bi bi-broom"></i>
                    <h3>Clean Installation</h3>
                    <p>Dust containment and thorough cleanup to maintain your sterile environment.</p>
                </div>
                <div class="why-card">
                    <i class="bi bi-geo-alt"></i>
                    <h3>Local Service</h3>
                    <p>Quick response from our Boca Raton facility for ongoing service needs.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="faq-section" style="padding: 80px 0; background: #f8f8f8;">
        <div class="container">
            <h2 class="section-title" style="text-align: center;">Frequently Asked Questions</h2>
            <div style="max-width: 800px; margin: 0 auto;">
                <div class="accordion">
                    <div class="accordion-item">
                        <button class="accordion-header">
                            <span>Are quartz countertops sanitary enough for medical facilities?</span>
                            <i class="bi bi-plus"></i>
                        </button>
                        <div class="accordion-content">
                            <p>Quartz is one of the most hygienic surface options available. Its non-porous surface prevents bacteria, mold, and mildew from penetrating, and it can be cleaned with hospital-grade disinfectants without damage. Many healthcare facilities choose quartz specifically for its sanitary properties.</p>
                        </div>
                    </div>
                    <div class="accordion-item">
                        <button class="accordion-header">
                            <span>Can quartz withstand medical-grade disinfectants?</span>
                            <i class="bi bi-plus"></i>
                        </button>
                        <div class="accordion-content">
                            <p>Yes, quartz resists most commercial cleaning agents and hospital-grade disinfectants including diluted bleach, quaternary ammonium compounds, and hydrogen peroxide solutions. Unlike natural stone, it won't etch or degrade from chemical cleaners.</p>
                        </div>
                    </div>
                    <div class="accordion-item">
                        <button class="accordion-header">
                            <span>How quickly can you install countertops in an active clinic?</span>
                            <i class="bi bi-plus"></i>
                        </button>
                        <div class="accordion-content">
                            <p>We understand that medical facilities can't afford extended downtime. Most exam room or reception area installations are completed in a single day. We can work after-hours or on weekends and install in phases so you never close your entire practice.</p>
                        </div>
                    </div>
                    <div class="accordion-item">
                        <button class="accordion-header">
                            <span>What countertop style works best for medical offices?</span>
                            <i class="bi bi-plus"></i>
                        </button>
                        <div class="accordion-content">
                            <p>Clean, light-colored quartz is popular in medical settings because it looks professional, brightens the space, and makes it easy to spot spills or contamination. Solid whites and subtle veining are the most requested for exam rooms and nurse stations.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="cta-section" id="contact">
        <div class="container">
            <h2>Ready to Upgrade Your Practice?</h2>
            <p>Contact us today for a free consultation. We'll work around your patient schedule to deliver hygienic, beautiful surfaces for your healthcare facility.</p>
            <a href="contact" class="btn">Request Free Estimate</a>
        </div>
    </section>

    <?php include 'includes/footer.php'; ?>
</body>
</html>
